### Olá, sou o Renan

Estou no primeiro semestre de Análise e Desenvolvimento de Sistemas e estudando Python por conta própria pela Alura, em paralelo com a faculdade.

Esse perfil reúne os exercícios e pequenos projetos que venho fazendo enquanto aprendo. Nada aqui é avançado ainda, mas é tudo escrito e entendido por mim.

**O que estou estudando agora:** lógica de programação, funções, estruturas de dados (listas, tuplas, dicionários) e comecei a praticar testes simples com pytest.

**Alguns projetos:**
- [pyinvest-simulador](https://github.com/RenanAC0/pyinvest-simulador) — simulador de investimentos comparando CDB, LCI, poupança e FII
- [cofre-digital](https://github.com/RenanAC0/cofre-digital) — jogo de adivinhar senha
- [jogo-adivinhacao-python](https://github.com/RenanAC0/jogo-adivinhacao-python) — jogo de adivinhar número
- [exercicios-python-ads](https://github.com/RenanAC0/exercicios-python-ads) — exercícios da faculdade organizados por tema

Pode entrar em contato comigo pelo [LinkedIn](www.linkedin.com/in/renan-araujo-a395723a5).

